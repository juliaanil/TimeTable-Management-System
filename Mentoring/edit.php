<?php 
    ...
    $bid = $_GET['bid'];
    $sql1="select * from tbl_mentoring where bid=$bid";
    ...
    $sql_mentor="Select * from tbl_mentoringfaculty where bid=$bid";
        $mentors = $db->query($sql_mentor);
        // $total=mysqli_num_rows($mentors);
        if($mentors->num_rows>0)
        {
            $mentor = array();
            while($mentor_row=$mentors->fetch_assoc())
            {
                $fid=$mentor_row['fid'];
                $mentor[] = $fid;  
            }
	...
	{    
           ...
           // $newpid=$_POST['pName'];
            ...
            $newsession=$_POST['session'];
                ...
                $sql = "update tbl_mentoring set day = '$newday' where bid = '$bid'";
                if($db->query($sql) == TRUE)
                { 
                $_SESSION['msg'] = "Updation successful";
...
<body id="page-top">
  ...
  <!-- Divider -->
								...
								<div class="card mt-4  bg-white	">
									...
									<div class="card-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="form-group">
												 <label for="exampleInputEmail1">Edit mentors</label> <br>
                                               
                                                 <select name = 'faculty[]' multiple size = 6>    
													<option value=""></option>
                                                    <?php



                                                 $sql_faculty = "select * from tbl_faculty where fStatus=1";
                                                        ...
                                                        if( $result_faculty->num_rows > 0 )
                                                        {
                                                            ...
                                                            { 
                                                                ...
                                                                while($mentor_row=$mentors->fetch_assoc())
                                                                {
                                                                    $fidselected=$mentor_row['fid'];
                                                                    $mentor[] = $fidselected;
                                                                    if($fidselected==$curfacultyid)
                                                                    {
                                                                     ?>
                                                            
                                                               <option value="<?php echo $faculty_row["fid"]; ?>" <?php if($faculty_row['fid']==$fidselected){ ?>selected="selected"<?php } ?>> <?php echo $faculty_row["fName"] ;?> </option>
                                                                      <?php 
                                                                      break;
                                                                    }
                                                        ...
                                                        $ids = implode(', ', $mentor);
                                                $sql_faculty = "select * from tbl_faculty where fid NOT IN ($ids)";
                                                // $sql_faculty = "select * from tbl_faculty where fid NOT IN (select fid from tbl_faculty where fid IN ('$ids'))";
        ...
        {
            $sql1 = "Delete from tbl_mentoringfaculty where bid=$bid";
            ...
            {
                 echo "You selected $faculty<br/>";

                 $sql1 = "insert into tbl_mentoringfaculty(bid,fid) values ($bid,$faculty)";
